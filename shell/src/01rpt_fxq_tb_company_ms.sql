...
--  rpt_fxq_manual_company_ms(pt分区字段保留，不使用)
--  输出:  
...
alter table rpt_fxq_tb_company_ms truncate partition pt{workday}000000;

--  未将空值处理为-9999或＠N,用来上报前整理数据的临时表
drop table if exists s_rpt_fxq_tb_company_ms;

create temporary table s_rpt_fxq_tb_company_ms select * from rpt_fxq_tb_company_ms where 1 <> 1;

insert into s_rpt_fxq_tb_company_ms (
    head_no, 
    company_code1, 
    company_code2, 
    company_name, 
...
from ods_cthx_web_org_dpt partition(pt{workday}000000) d
    inner join  rpt_fxq_manual_company_ms c 
