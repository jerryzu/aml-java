...
alter table rpt_fxq_tb_ins_rsur_ms truncate partition pt{workday}000000;

--  未将空值处理为-9999或＠N,用来上报前整理数据的临时表
drop table if exists s_rpt_fxq_tb_ins_rsur_ms;

create temporary table s_rpt_fxq_tb_ins_rsur_ms select * from rpt_fxq_tb_ins_rsur_ms where 1 <> 1;

INSERT INTO s_rpt_fxq_tb_ins_rsur_ms(
        company_code1,
        company_code2,
        company_code3,
        company_code4,
